...
/// When writing a match expression against `TransformationDownloadArtifactType`, it is important to ensure
...
/// calling `as_str` on `TransformationDownloadArtifactType::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for TransformationDownloadArtifactType {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => TransformationDownloadArtifactType::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
