            ...
            loop {
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => match key.to_unescaped()?.as_ref() {
                        "webLink" => {
                            builder = builder.set_web_link(crate::protocol_serde::shape_web_link::de_web_link(tokens)?);
                        }
                        "moduleLink" => {
                            builder = builder.set_module_link(crate::protocol_serde::shape_module_link::de_module_link(tokens)?);
                        }
