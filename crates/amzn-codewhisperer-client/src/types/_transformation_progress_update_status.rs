...
/// When writing a match expression against `TransformationProgressUpdateStatus`, it is important to ensure
...
/// calling `as_str` on `TransformationProgressUpdateStatus::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for TransformationProgressUpdateStatus {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => TransformationProgressUpdateStatus::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
...
impl TransformationProgressUpdateStatus {
    ...
    pub const fn values() -> &'static [&'static str] {
        &["AWAITING_CLIENT_ACTION", "COMPLETED", "FAILED", "IN_PROGRESS", "PAUSED", "SKIPPED"]
    }
}
