...
impl Customization {
    ...
    pub fn load_selected(state: &State) -> Result<Option<Self>, crate::fig_settings::Error> {
        state.get(CUSTOMIZATION_STATE_KEY)
    }

    /// Save the currently selected customization to state
    pub fn save_selected(&self, state: &State) -> Result<(), crate::fig_settings::Error> {
        state.set_value(CUSTOMIZATION_STATE_KEY, serde_json::to_value(self)?)
    }

    /// Delete the currently selected customization from state
    pub fn delete_selected(state: &State) -> Result<(), crate::fig_settings::Error> {
