...
use crate::fig_util::env_var::{
    Q_BUNDLE_METADATA_PATH,
    Q_PARENT,
};
use crate::fig_util::linux::PACKAGE_NAME;
...
pub fn resources_path() -> Result<PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(all(unix, not(target_os = "macos")))] {
            ...
            Ok(crate::fig_util::app_bundle_path().join(crate::fig_util::macos::BUNDLE_CONTENTS_RESOURCE_PATH))
        }
    }
}
...
pub fn resources_path_ctx<Ctx: EnvProvider + PlatformProvider>(ctx: &Ctx) -> Result<PathBuf> {
    ...
    match os {
        crate::fig_os_shim::Os::Mac => {
            Ok(crate::fig_util::app_bundle_path().join(crate::fig_util::macos::BUNDLE_CONTENTS_RESOURCE_PATH))
        },
        crate::fig_os_shim::Os::Linux => {
...
pub fn local_webview_data_dir<Ctx: FsProvider + EnvProvider + PlatformProvider>(ctx: &Ctx) -> Result<PathBuf> {
    ...
    Ok(local_data_dir(ctx)?.join(crate::fig_util::consts::linux::DESKTOP_APP_WM_CLASS))
}
