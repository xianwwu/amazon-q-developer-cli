...
use crate::fig_install::download::download_file;
use crate::fig_install::index::UpdatePackage;
use crate::fig_install::{
    Error,
    UpdateStatus,
};
use crate::fig_util::consts::{
    APP_BUNDLE_ID,
    CLI_BINARY_NAME,
};
use crate::fig_util::macos::BUNDLE_CONTENTS_MACOS_PATH;
use crate::fig_util::{
    APP_BUNDLE_NAME,
    directories,
};
...
pub(crate) async fn update(
    ...
    let cli_path = crate::fig_util::app_bundle_path()
    ...
    let installed_app_path = if same_bundle_name {
        crate::fig_util::app_bundle_path()
    } else {
    ...
    if !same_bundle_name {
        cmd.arg("--delete-bundle").arg(crate::fig_util::app_bundle_path());
    }
...
pub(crate) async fn uninstall_desktop(ctx: &crate::fig_os_shim::Context) -> Result<(), Error> {
    // TODO:
    ...
    if let Ok(fig_data_dir) = directories::fig_data_dir() {
        let state = crate::fig_settings::state::get_string("anonymousId").unwrap_or_default();
    ...
    let app_path = crate::fig_util::app_bundle_path();
    if app_path.exists() {
...
mod test {
    ...
    async fn test_download_dmg() -> Result<(), Error> {
        let index = crate::fig_install::index::pull(&Channel::Stable).await.unwrap();
