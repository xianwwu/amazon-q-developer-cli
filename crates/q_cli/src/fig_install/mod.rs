...
use crate::fig_util::PRODUCT_NAME;
...
pub fn get_max_channel() -> Channel {
    let state_channel = crate::fig_settings::state::get_string("updates.channel")
        .ok()
        .flatten()
        .and_then(|s| Channel::from_str(&s).ok())
        .unwrap_or(Channel::Stable);
    let manifest_channel = manifest().default_channel;
    let settings_channel = if crate::fig_settings::settings::get_bool_or("app.beta", false) {
        Channel::Beta
    } else {
        Channel::Stable
    };
...
pub async fn check_for_updates(ignore_rollout: bool) -> Result<Option<UpdatePackage>, Error> {
    ...
    let file_type = match (&manifest.variant, ctx.platform().os()) {
        (Variant::Full, crate::fig_os_shim::Os::Linux) => match index::get_file_type(&ctx, &manifest.variant).await {
...
pub async fn update(
    ...
    if let Some(update) = check_for_updates(ignore_rollout).await? {
        ...
        let lock_file = crate::fig_util::directories::update_lock_path(&ctx)?;
