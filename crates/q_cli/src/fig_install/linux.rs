...
async fn replace_bins(bin_dir: &Path) -> Result<(), Error> {
    let local_bin = crate::fig_util::directories::home_local_bin()?;
...
pub(crate) async fn update(
    ...
    match &manifest().variant {
        crate::fig_util::manifest::Variant::Full => update_full(update_package, tx, interactive, relaunch_dashboard).await,
        crate::fig_util::manifest::Variant::Minimal => {
            update_minimal(update_package, tx, interactive, relaunch_dashboard).await
        },
        crate::fig_util::manifest::Variant::Other(other) => Err(Error::UnsupportedVariant(other.clone())),
    }
}
...
pub(crate) async fn update_minimal(
    ...
    let local_bin = crate::fig_util::directories::home_local_bin()?.canonicalize()?;
...
async fn update_full_ctx(
    ...
    let lock_file_path = crate::fig_util::directories::update_lock_path(ctx)?;
...
mod tests {
    ...
    use crate::fig_util::directories::home_dir;
    ...
    async fn test_appimage_updates_successfully() {
        ...
        assert!(
            !ctx.fs().exists(crate::fig_util::directories::update_lock_path(&ctx).unwrap()),
