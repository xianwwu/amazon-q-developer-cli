...
use crate::fig_util::system_info::get_system_id;
...
static RELEASE_URL: LazyLock<Url> = LazyLock::new(|| {
    match std::env::var("Q_DESKTOP_RELEASE_URL") {
        Ok(s) => Url::parse(&s),
        Err(_) => match crate::fig_settings::settings::get_string("install.releaseUrl") {
...
pub(crate) async fn get_file_type(ctx: &Context, variant: &Variant) -> Result<FileType, Error> {
    match ctx.platform().os() {
        crate::fig_os_shim::Os::Mac => Ok(FileType::Dmg),
        crate::fig_os_shim::Os::Linux => match variant {
...
mod tests {
    ...
    fn load_test_index() -> Index {
        serde_json::from_str(include_str!("test-index.json")).unwrap()
    }
