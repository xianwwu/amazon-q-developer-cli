...
pub enum ChatState {
    ...
    /// Exit the chat.
    Exit,
    /// Execute a command directly from a string.
    ExecuteCommand(String),
    /// Execute an already parsed command directly.
    ExecuteParsedCommand(command::Command),
}
...
impl ChatContext {
        ...
        loop {
            ...
            let result = match chat_state {
                ChatState::PromptUser {
                    ...
                    if !self.interactive {
                        info!("Non-interactive mode detected, exiting");
                        return Ok(());
                    }
                    debug!("Prompting user for input");
                ...
                ChatState::ExecuteCommand(command_str) => {
                    info!("Executing command: {}", command_str);
                    self.execute_command(command_str).await
                },
                ChatState::ExecuteParsedCommand(command) => {
                    info!("Executing parsed command: {:?}", command);
                    self.execute_parsed_command(command).await
                },
                ChatState::Exit => {
                    info!("Exit state detected, terminating chat");
                    return Ok(());
                },
            };
    ...
    async fn handle_state_execution_result(
        ...
        match result {
            Ok(state) => {
                debug!("handle_state_execution_result received successful state: {:?}", state);
                Ok(state)
            },
            Err(e) => {
    ...
    async fn handle_input(
        ...
        info!("handle_input called with input: {}", user_input);

        let command_result = Command::parse(&user_input, &mut self.output);
        debug!("Command parsed as: {:?}", command_result);
        ...
        Ok(match command {
            Command::Ask { prompt } => {
                ...
                debug!("Processing as regular user input");
                self.tool_use_status = ToolUseStatus::Idle;

                if pending_tool_index.is_some() {
                    info!("Abandoning tool use due to user rejection");
                    self.conversation_state.abandon_tool_use(tool_uses, user_input);
                } else {
                    debug!("Setting next user message");
            ...
            Command::Compact {
                ...
                info!(
                    "Compact command detected with prompt: {:?}, show_summary: {}, help: {}",
                    prompt, show_summary, help
                );
                self.compact_history(Some(tool_uses), pending_tool_index, prompt, show_summary, help)
                    .await?
            },
            Command::Help => {
                info!("Help command detected, displaying help text");
                execute!(self.output, style::Print(HELP_TEXT))?;
                ChatState::PromptUser {
                    tool_uses: Some(tool_uses),
                    pending_tool_index,
            ...
            Command::Quit => {
                info!("Quit command detected, exiting chat");
                debug!("Returning ChatState::Exit");
                ChatState::Exit
            },
            Command::Profile { subcommand } => {
                info!("Profile command detected with subcommand: {:?}", subcommand);
...
impl ChatContext {
    ...
    async fn parse_command(&mut self, command_str: &str) -> Result<Option<command::Command>, ChatError> {
        // Log the command being parsed
        info!("parse_command called with command: {}", command_str);

        // Special handling for critical commands
        if command_str.trim() == "/quit" {
            info!("Detected quit command, will return Command::Quit");
            return Ok(Some(command::Command::Quit));
        }

        // Handle compact command with its various options
        if command_str.starts_with("/compact") {
            info!("Detected compact command, parsing options");
            ...
            // Simple parsing of compact command options
            let parts: Vec<&str> = command_str.split_whitespace().collect();

            // Check for flags and prompt
            if parts.len() > 1 {
                for i in 1..parts.len() {
                    match parts.get(i) {
                        ...
                        _ => {
                            // Collect everything else as the prompt
                            if prompt.is_none() {
                                prompt = Some(parts[i..].join(" "));
                                break;
                            }
                        },
                    }
            ...
            info!(
                "Creating Command::Compact with prompt: {:?}, show_summary: {}, help: {}",
                prompt, show_summary, help
            );

            return Ok(Some(command::Command::Compact {
                prompt,
                show_summary,
                help,
            }));
        ...
        // Handle help command
        if command_str.trim() == "/help" {
            info!("Detected help command, will return Command::Help");
            return Ok(Some(command::Command::Help));
        }

        // Parse the command string using the standard parser
        let command_result = command::Command::parse(command_str, &mut self.output);

        // If the command is valid, return it
        if let Ok(command) = command_result {
            debug!("Command parsed successfully: {:?}", command);
            Ok(Some(command))
        } else {
            // If the command is invalid, display an error message
            warn!("Failed to parse command: {:?}", command_result.err());
    ...
    /// Execute a command directly from a string
    async fn execute_command(&mut self, command_str: String) -> Result<ChatState, ChatError> {
        // Log the command being executed
        info!("execute_command called with command: {}", command_str);

        // Parse the command string
        let parsed_command = self.parse_command(&command_str).await?;

        // If the command was parsed successfully, execute it
        if let Some(command) = parsed_command {
            debug!("Command parsed successfully, executing: {:?}", command);
            return self.execute_parsed_command(command).await;
        } else {
            // If the command is invalid, return to prompt user
            Ok(ChatState::PromptUser {
                tool_uses: None,
                pending_tool_index: None,
                skip_printing_tools: false,
    ...
    /// Execute an already parsed command
    /// 
    /// This method takes a Command enum and executes it, returning the appropriate ChatState.
    /// It handles special commands directly and falls back to string conversion for other commands.
    /// 
    /// # Arguments
    /// 
    /// * `command` - The parsed Command enum to execute
    /// 
    /// # Returns
    /// 
    /// * `Result<ChatState, ChatError>` - The resulting state after command execution
    async fn execute_parsed_command(&mut self, command: command::Command) -> Result<ChatState, ChatError> {
        // Log the parsed command being executed
        info!("execute_parsed_command called with command: {:?}", command);

        // Handle special commands directly
        match &command {
            command::Command::Quit => {
                info!("Executing Command::Quit, returning ChatState::Exit");
                Ok(ChatState::Exit)
            },
            command::Command::Help => {
                info!("Executing Command::Help, returning ChatState::DisplayHelp");
            ...
            command::Command::Compact {
                ...
                info!("Executing Command::Compact, returning ChatState::Compact");
            ...
            _ => {
                // For other commands, convert back to string and use handle_input
                // This is a temporary solution until all commands are migrated to use execute_parsed_command
                // directly
                let command_str = format!("/{:?}", command)
                    .replace("\"", "")
                    .replace("(", " ")
                    .replace(")", "");
                debug!("Converting command to string for handle_input: {}", command_str);

                // Execute the command using the existing handle_input method
                let result = self.handle_input(command_str, None, None).await;

                // Log the result
                match &result {
                    Ok(state) => debug!("handle_input returned state: {:?}", state),
                    Err(err) => warn!("handle_input returned error: {:?}", err),
                }
